...
var __slice = [].slice;
  ...
  StorageFreak = (function() {
    var absolute_sort, fuzzy_sort, lexicographical_sort, prefix_sort, token_sort;

    absolute_sort = function(query, a, b) {
      var re_abs;
      re_abs = this._createAccentIndependentRE(query, 'absolute');
      return (function(abs_match_a, abs_match_b) {
        if (abs_match_a) {
          return -1;
        }
        if (abs_match_b) {
      ...
      })(this.config.match_func(re_abs, a.name), this.config.match_func(re_abs, b.name));
    };

    token_sort = function(query, a, b) {
      var re_t;
      re_t = this._createAccentIndependentRE(query, 'token');
      return (function(token_match_a, token_match_b) {
        if (token_match_a && !token_match_b) {
          return -1;
        }
        if (token_match_b && !token_match_a) {
          return 1;
        }
      })(this.config.match_func(re_t, a.name), this.config.match_func(re_t, b.name));
    };

    prefix_sort = function(query, a, b) {
      var re_p;
      re_p = this._createAccentIndependentRE(query, 'prefix');
      return (function(prefix_match_a, prefix_match_b) {
        if (prefix_match_a && !prefix_match_b) {
          return -1;
        }
        if (prefix_match_b && !prefix_match_a) {
          return 1;
        }
      })(this.config.match_func(re_p, a.name), this.config.match_func(re_p, b.name));
    };
    ...
    fuzzy_sort = function(query, a, b) {
      var abs_result, prefix_result, token_result;
      abs_result = absolute_sort.call(this, query, a, b);
      if (abs_result) {
        return abs_result;
      }
      token_result = token_sort.call(this, query, a, b);
      if (token_result) {
        return token_result;
      }
      prefix_result = prefix_sort.call(this, query, a, b);
      if (prefix_result) {
    ...
    StorageFreak.prototype._searchData = function(query, options) {
      ...
      re = this._createAccentIndependentRE(query, this.config.search_type);
      _ref = this._data;
      for (id in _ref) {
        name = _ref[id];
      ...
      results = results.sort(((function(_this) {
        return function(a, b) {
    ...
    StorageFreak.prototype._getRemoteData = function(query, options) {
      ...
      this.XHR_dfd = $.getJSON(options.url || this.config.url, params);
      this.XHR_dfd.then((function(_this) {
        return function(json_data) {
          return _this._getRemoteSuccess(json_data, dfd, options);
        };
      })(this), (function(_this) {
        return function(xhr) {
    ...
    StorageFreak.prototype._getRemoteSuccess = function(json_data, dfd, options) {
      this._data = $.extend({}, this._data, this._parseResponseData(json_data));
      this._updateDB(this._data);
    ...
    StorageFreak.prototype._updateDB = function(data) {
      this._set(this._data_key, data);
      this._set(this._timestamp_key, new Date().getTime());
      return this._trigger('dbupdated');
    };

    StorageFreak.prototype._createAccentIndependentRE = function(query, type) {
...
var __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
  ...
  Selectorablium = (function() {
    ...
    Selectorablium.prototype._createTemplate = function(query) {
      var class_name, item, items_templates, text, _i, _len, _ref;
      items_templates = [];
